    ...
    if (d.type === "chore") {
      ...
      return this._modal(chore ? "Edit chore" : "Add a chore", "Set who owns it, when it repeats, and what it’s worth.", `
        <form id="chore-form" class="form">
          ...
          <label><span>List</span><select name="assignee_id"><option value="">Up for grabs</option>${this._data.people.map(person => `<option value="${this._escape(person.id)}" ${target === person.id ? "selected" : ""}>${this._escape(person.name)}</option>`).join("")}</select></label>
